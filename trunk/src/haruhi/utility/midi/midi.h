...
{
  public:
	typedef Core::Event::Event::Timestamp Timestamp;
...
class ChannelEvent: public Event
{
  ...
  public:
	ChannelEvent (Timestamp timestamp, Channel channel):
		Event (timestamp),
		_channel (channel)
	{ }
...
class NoteEvent: public ChannelEvent
{
  ...
  public:
	NoteEvent (Timestamp timestamp, Channel channel, Note note, Velocity velocity):
		ChannelEvent (timestamp, channel),
		_note (note),
		_velocity (velocity)
	{ }
	...
	/**
	 * If velocity is positive, it is NOTE-ON event. Otherwise (0 or negative)
	 * it is NOTE-OFF event.
	 */
	Velocity
	velocity() const
	{
...
class ControllerEvent: public ChannelEvent
{
  ...
  public:
	ControllerEvent (Timestamp timestamp, Channel channel, Controller controller, Value value):
		ChannelEvent (timestamp, channel),
...
class PitchbendEvent: public ChannelEvent
{
  ...
  public:
	PitchbendEvent (Timestamp timestamp, Channel channel, Value value):
		ChannelEvent (timestamp, channel),
		_value (value)
	{ }
