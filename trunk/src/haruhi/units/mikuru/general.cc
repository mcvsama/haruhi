...
General::load_params()
{
	...
	_proxy_volume->set_value (p.volume);
	_proxy_detune->set_value (p.detune);
	_proxy_panorama->set_value (p.panorama);
	_proxy_stereo_width->set_value (p.stereo_width);
	_proxy_input_volume->set_value (p.input_volume);
